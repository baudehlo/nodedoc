...
var node_dir = process.env.NODE_DOC_DIR || '/usr/local/share/node';

var page = process.argv.pop();

if (process.argv.length === 1 || !page) {
    process.stderr.write("Usage: nodedoc [page]\n");
    process.exit(1);
}

var file = page.toLowerCase() + '.markdown';

var full_path = path.join(node_dir, file);

if (!path.existsSync(full_path)) {
    process.stderr.write("No such man page: " + page + "\n");
    process.stderr.write("Have you checked the markdown files are in " + node_dir + "?\n");
    process.exit(1);
}

var rs = fs.createReadStream(full_path);
var md_string = '';
...
rs.on('end', function () {
    var md = new Markdown();
    var tree = md.parse(md_string);
    render_tree_to_pod(tree);
});
...
function render_tree_to_pod(tree) {
    if (!(tree instanceof Array)) {
        return write(tree);
    }
    var type = tree.shift();
    render_item_to_pod(type, tree);
}

function render_kids_to_pod(tree) {
    for (var i=0,l=tree.length; i<l; i++) {
        render_tree_to_pod(tree[i]);
    }
}

var util = require('util');

function render_item_to_pod(type, tree) {
    switch (type) {
        case 'markdown': write("=pod\n\n");
                         render_kids_to_pod(tree);
                         break;
        case 'header':   write("=head" + tree[0].level + " " + tree[1] + "\n\n");
                         break;
        case 'para':     write_para(tree);
                         break;
        case 'inlinecode':
                         write("C<<< ");
                         write(tree[0]);
                         write(" >>>");
                         if (tree.length > 1) {
                             process.stderr.write("ERROR: inline code too long\n");
                         }
                         break;
        case 'code_block':
                         write(indent(tree[0]));
                         write("\n\n");
                         if (tree.length > 1) {
                             process.stderr.write("ERROR: expecting code_block to be 1 block\n");
                         }
                         break;
        case 'bulletlist':
                         write("=over 4\n\n");
        ...
        case 'link':
                         write("L<");
                         write(tree[1] + '|' + tree[0].href.replace(/#/, '/'));
                         write(">");
                         break;
        ...
        default:         process.stderr.write("unknown type: " + type + ': ' + util.inspect(tree, null, null));
    }
}
 ...
 **/
Markdown.prototype.parse = function( source, dialect ) {
 ...
 **/
Markdown.prototype.toHTML = function toHTML( source , dialect ) {
  var input = Markdown.prototype.toHTMLTree( source , dialect );

  return Markdown.prototype.renderJsonML( input );
}
 ...
 *  [[parse]].
 **/
Markdown.prototype.toHTMLTree = function toHTMLTree( input, dialect ) {
 ...
 *    root itself.
 */
Markdown.prototype.renderJsonML = function( jsonml, options ) {
  options = options || {};
